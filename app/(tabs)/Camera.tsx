...
async function ocrImageBase64(base64: string) {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `API错误 (${res.status}): ${errorData.error?.message || res.statusText}`
    );
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inlineMatch) {
      ...
      if (
        ...
        price >= 30 &&
        price <= 10000 &&
        !skipKeywords.some((word) => name.includes(word))
      ) {
...
export default function CameraScreen() {
  ...
  if (hasPermission === null) {
    return (
      <ThemedView style={styles.container}>
        <ThemedText>カメラの権限をリクエストしています...</ThemedText>
      </ThemedView>
    );
  }
  if (hasPermission === false) {
    return (
      <ThemedView style={styles.container}>
        <ThemedText>
          カメラへのアクセスが許可されていません。設定で許可してください。
        </ThemedText>
      </ThemedView>
    );
  }
  ...
  return (
    <ThemedView style={styles.container}>
      {/* 顶部按钮栏 */}
      <RNView style={styles.topButtonContainer}>
        {/* 左上角返回按钮 */}
        <TouchableOpacity
      ...
      >
        <RNView style={styles.modalOverlay}>
          <RNView style={styles.modalContent}>
            ...
            {/* 说明内容 */}
            <ThemedText style={styles.instructionTitle}>撮影のコツ</ThemedText>
